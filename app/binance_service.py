...
def get_withdraw_history(asset: str = None, start_time: int = None, end_time: int = None):
    client = get_binance_client()
    return client.withdraw_history(asset=asset, startTime=start_time, endTime=end_time)

def get_all_deposits(asset: str = None, earliest_date: str = "2017-07-01", latest_date: str = None) -> dict:
    """
    Fetches all deposits from Binance, paginating 90-day windows, and returns all results in one call.
    Iterates through all windows until earliest_date, even if some windows are empty.
    ...
    while True:
        ...
        print(f"Fetching deposits: page {page}, {start_time.date()} to {end_time.date()} ...")
        deposits = client.deposit_history(asset=asset, startTime=start_time_ms, endTime=end_time_ms)
        print(f"  Found {len(deposits) if deposits else 0} deposits in this window.")
        if deposits:
...
def get_all_withdrawals(asset: str = None, earliest_date: str = "2017-07-01", latest_date: str = None) -> dict:
    client = get_binance_client()
    if latest_date:
        latest_dt = datetime.strptime(latest_date, "%Y-%m-%d").replace(tzinfo=timezone.utc)
    else:
    ...
    while True:
        ...
        end_time_ms = int(end_time.timestamp() * 1000)
        start_time_ms = int(start_time.timestamp() * 1000)
        print(f"Fetching withdrawals: page {page}, {start_time.date()} to {end_time.date()} ...")
        withdrawals = client.get_withdraw_history(asset=asset, startTime=start_time_ms, endTime=end_time_ms)
        print(f"  Found {len(withdrawals) if withdrawals else 0} withdrawals in this window.")
        if withdrawals:
...
def get_lending_interest_history(lending_type: str = "DAILY", asset: str = None, start_time: int = None, end_time: int = None, limit: int = 1000):
    """
    Fetches interest history from Binance Savings (Flexible, Activity, or Customized Fixed).
    """
    client: Spot = get_binance_client()
    params = {
...
def get_flexible_redemption_record(
    ...
    """
    Fetches Flexible Redemption Record from Binance Simple Earn using binance-connector.
    """
    client = get_binance_client()
    params = {
        "current": current,
